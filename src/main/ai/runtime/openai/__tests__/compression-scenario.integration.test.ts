...
function checkKeywords(text: string): { key: string; label: string; found: boolean }[] {
  return TRACK_KEYWORDS.map((kw) => {
    const found = kw.caseInsensitive ? text.toLowerCase().includes(kw.key.toLowerCase()) : text.includes(kw.key);
...
function logKeywordComparison(label: string, beforeText: string, afterText: string): void {
  ...
  log(`\n  ── 关键信息追踪: ${label} ──`);
  log(`  ${'关键词'.padEnd(14)}${'标签'.padEnd(12)}${'压缩前'.padEnd(8)}${'压缩后'.padEnd(8)}${'状态'}`);
  log(`  ${'─'.repeat(50)}`);
...
function logSnapshot(label: string, snapshot: ContextSnapshot): void {
  ...
  if (snapshot.lastSummary) {
    // 清洗显示
    const cleanText = snapshot.lastSummary.summaryText.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
    log(`\n  ── Summary (endSeq=${snapshot.lastSummary.endSeq}) ──`);
    log(`    seqs: [${snapshot.lastSummary.summarizedSeqs.join(', ')}]`);
    log(`    tokens: ${snapshot.lastSummary.originalTokens} → ${snapshot.lastSummary.summaryTokens}`);
    ...
    for (let i = 0; i < conversations.length; i++) {
      ...
      if (compressed) {
        ...
        if (after.lastSummary) {
          const cleanSummary = after.lastSummary.summaryText.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
          log(`\n  ── Summary 内容 ──`);
...
describe.skipIf(!RUN)('Session 压缩场景：工具调用 + 压缩', () => {
   ...
   */
  it('工具调用 + 流式压缩：上下文含 function_call 时的完整压缩链路', { timeout: 360_000 }, async () => {
    sessionId = uid();
    ...
    const conversations = [
      ...
      {
        input: '请回顾一下：我叫什么名字？之前 17+25 的结果是什么？北京天气怎样？12×10 等于多少？',
        tag: 'R7-验证回忆',
        expectTool: false,
        streaming: false
      },
    ...
    for (let i = 0; i < conversations.length; i++) {
      ...
      if (streaming) {
        ...
        const result = await runtime.runStream(input, { streaming: true }, (chunk: StreamChunk) => {
          chunks.push(chunk);
      ...
      if (compressed) {
        ...
        toolLog(`\n  ── 关键信息追踪: 第 ${turn} 轮压缩 ──`);
        toolLog(`  ${'关键词'.padEnd(14)}${'标签'.padEnd(14)}${'压缩前'.padEnd(8)}${'压缩后'.padEnd(8)}${'状态'}`);
        toolLog(`  ${'─'.repeat(54)}`);
        let lostCount = 0;
        ...
        if (after.lastSummary) {
          ...
          toolLog(`    tokens: ${after.lastSummary.originalTokens} → ${after.lastSummary.summaryTokens}`);
    ...
    if (finalSnapshot.lastSummary) {
      const cleanSummary = finalSnapshot.lastSummary.summaryText.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
      toolLog(`\n  ── 最终 Summary ──`);
      toolLog(`    endSeq: ${finalSnapshot.lastSummary.endSeq}`);
      toolLog(`    seqs: [${finalSnapshot.lastSummary.summarizedSeqs.join(', ')}]`);
      toolLog(`    tokens: ${finalSnapshot.lastSummary.originalTokens} → ${finalSnapshot.lastSummary.summaryTokens}`);
      toolLog(
        `    text:\n${cleanSummary
          .split('\n')
          .map((l) => `      ${l}`)
