...
// ===== Hoisted mocks =====
const { mockOn, mockExecute, mockQuery, mockQueryOne, mockTransaction } = vi.hoisted(() => {
  ...
  describe('消息入队与刷新', () => {
    it('通过事件监听入队消息', async () => {
      ...
      const messageHandler = mockOn.mock.calls.find(
        (call) => call[0] === StreamEventType.MESSAGE
      )?.[1]
  ...
  describe('cleanOldMessages', () => {
    it('清理旧消息', async () => {
      ...
      expect(mockExecute).toHaveBeenCalledWith(
        expect.stringContaining('DELETE FROM stream_messages'),
        ['s1', 500, 's1']
      )
    })
  })
