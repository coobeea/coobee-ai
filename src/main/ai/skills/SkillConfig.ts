 ...
 */
export function loadSkillConfigs(configDir: string): SkillConfigMap {
  const filePath = skillConfigPath(configDir)
...
export function getSkillConfig(
  ...
  const configs = loadSkillConfigs(configDir)
  return configs[skillName]
}

/** skills.json5 文件路径 */
export function skillConfigPath(configDir: string): string {
  return path.join(configDir, SKILL_CONFIG_FILE_NAME)
}

/** 确保 skills.json5 存在，不存在则创建模板 */
export function ensureSkillConfigFile(configDir: string): void {
  const filePath = skillConfigPath(configDir)
  if (fs.existsSync(filePath)) return
