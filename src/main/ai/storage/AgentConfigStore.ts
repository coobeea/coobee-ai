...
export class AgentConfigStore implements IAgentConfigStore {
   ...
   * 保存 Agent 配置
   */
  async saveConfig(config: Omit<AgentConfigData, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
  ...
  async getConfig(id: string): Promise<AgentConfigData | null> {
    const row = await this.db.queryOne<Record<string, unknown>>(`SELECT * FROM agent_configs WHERE id = ?`, [id]);
