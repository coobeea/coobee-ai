...
describe('LongTermMemoryStore', () => {
  ...
  describe('initialize', () => {
    it('创建 Schema', async () => {
      ...
      expect(mockExecute).toHaveBeenCalledWith(expect.stringContaining('CREATE TABLE IF NOT EXISTS long_term_memory'));
    });
  ...
  describe('retrieveMemories', () => {
    ...
    it('按类型过滤', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('type = ?'), [LongTermMemoryType.EPISODIC]);
    });
    ...
    it('按关键词过滤', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('content LIKE ?'), ['%TypeScript%', '%Vue%']);
    });
  ...
  describe('deleteMemory', () => {
    it('删除成功返回 true', async () => {
      ...
      expect(result).toBe(true);
      expect(mockExecute).toHaveBeenCalledWith(expect.stringContaining('DELETE FROM long_term_memory WHERE id = ?'), [
        'm1'
      ]);
    });
  ...
  describe('cleanupOldMemories', () => {
    it('清理低重要性过期记忆', async () => {
      ...
      expect(count).toBe(5);
      expect(mockExecute).toHaveBeenCalledWith(expect.stringContaining('importance < 5'), expect.any(Array));
    });
