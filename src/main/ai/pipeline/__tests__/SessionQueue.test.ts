...
describe('SessionQueue', () => {
  ...
  it('should enqueue and dequeue in FIFO order', async () => {
    await queue.enqueue('test-session', 'first');
    await queue.enqueue('test-session', 'second');
    await queue.enqueue('test-session', 'third');
  ...
  it('should peek without removing', async () => {
    await queue.enqueue('test-session', 'hello');
    expect(queue.peek()?.message).toBe('hello');
    expect(queue.length).toBe(1); // still in queue
  });

  it('should dequeueAll', async () => {
    await queue.enqueue('test-session', 'a');
    await queue.enqueue('test-session', 'b');
    await queue.enqueue('test-session', 'c');
  ...
  it('should drop oldest when cap reached (old policy)', async () => {
    const q = new SessionQueue('s1', { cap: 2, dropPolicy: 'old' });
    await q.enqueue('s1', 'first');
    await q.enqueue('s1', 'second');
    await q.enqueue('s1', 'third'); // should drop 'first'
  ...
  it('should drop new message when cap reached (new policy)', async () => {
    const q = new SessionQueue('s1', { cap: 2, dropPolicy: 'new' });
    await q.enqueue('s1', 'first');
    await q.enqueue('s1', 'second');
    await q.enqueue('s1', 'third'); // should be dropped
  ...
  it('should summarize oldest when cap reached (summarize policy)', async () => {
    const q = new SessionQueue('s1', { cap: 2, dropPolicy: 'summarize' });
    await q.enqueue('s1', 'first');
    await q.enqueue('s1', 'second');
    await q.enqueue('s1', 'third'); // 'first' goes to summary
  ...
  it('should return full state snapshot', async () => {
    await queue.enqueue('test-session', 'msg');
