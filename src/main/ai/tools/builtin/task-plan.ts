...
const paramsSchema = z.object({
  action: z
    .enum(['create', 'update_step', 'get', 'list', 'complete'])
  ...
  taskId: z
    ...
    .optional()
    .describe(
      'Task ID (kebab-case). Auto-generated for create if omitted. Required for update_step/get/complete.'
    ),
...
async function* handleCreate(
  ...
  const taskId =
    ...
    `${slugify(title)}-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}`
...
async function* handleList(
  ...
  const lines = tasks.map(
    (t) => `- **${t.title}** (\`${t.taskId}\`) — ${t.status}, ${t.steps} steps`
  )
