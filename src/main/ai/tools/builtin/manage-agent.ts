...
const paramsSchema = z.object({
  action: z.enum(['create', 'list', 'get', 'update', 'delete']).describe('Operation to perform on agents'),
  ...
  agentId: z
    ...
    .optional()
    .describe(
      'Agent ID (kebab-case). Required for create/get/update/delete. ' + 'Example: "code-reviewer", "contract-analyst"'
    ),

  // create 参数
  name: z.string().optional().describe('Display name for the agent (Chinese or English)'),
  description: z.string().optional().describe('One-line description of the agent purpose and capabilities'),
  instructions: z
    .string()
    .optional()
  ...
  skills: z.array(z.string()).optional().describe('List of skill names to associate with this agent'),
  ...
  createdBy: z
    ...
    .optional()
    .describe('Creator type: "user" for manual, "agent" for autonomous, "system" for built-in'),
...
async function* handleCreate(
  ...
  const definition = await store.create({
    ...
    thinkingLevel: params.thinkingLevel as 'minimal' | 'low' | 'medium' | 'high' | 'xhigh' | undefined,
    createdBy: (params.createdBy as 'user' | 'agent' | 'system') ?? 'agent',
...
async function* handleList(store: AgentStore): AsyncGenerator<ToolStreamUpdate, ToolResult, unknown> {
...
async function* handleDelete(
  ...
  if (existing?.createdBy === 'system') {
    return {
      success: false,
      error: { code: 'FORBIDDEN', message: `Built-in agent "${agentId}" cannot be deleted` }
    };
  }
