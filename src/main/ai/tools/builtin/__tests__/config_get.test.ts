...
describe('config_get 工具', () => {
  ...
  describe('获取配置', () => {
    it('获取全部配置', async () => {
      ...
      expect(result.llmContent).toContain('agents')
    ...
    it('获取指定配置节 — agents', async () => {
      const { result } = await executeTool({ key: 'agents' })

      expect(result.success).toBe(true)
      expect(result.llmContent).toContain('config.agents')
