...
export const globTool: ToolDefinition = {
  ...
  parameters: z.object({
    pattern: z
      .string()
      .describe(
        'Glob pattern to match file names (e.g. "*.ts", "**/*.test.ts", "package.json", "src/**/*.vue")'
      ),
    searchPath: z
      .string()
      .optional()
    ...
    maxResults: z
      .number()
      .optional()
      .describe(`Maximum results to return. Defaults to ${DEFAULT_MAX_RESULTS}`)
  }),
  ...
  execute: async function* (
    ...
    return {
      success: true,
      llmContent:
        `Found ${matches.length} files matching "${pattern}"${truncated}:\n\n` + lines.join('\n')
    }
  }
}
