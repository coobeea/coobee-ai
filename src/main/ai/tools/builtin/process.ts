...
export const processTool: ToolDefinition = {
  ...
  parameters: z.object({
    action: z.enum(['list', 'read_output', 'send_input', 'send_signal', 'kill']).describe('The action to perform'),
    processId: z.string().optional().describe('Target process ID (required for all actions except list)'),
    ...
    lastN: z.number().optional().describe('Number of recent output lines to return (for read_output). Defaults to all')
  }),
  ...
  execute: async function* (params: Record<string, unknown>): AsyncGenerator<ToolStreamUpdate, ToolResult, unknown> {
    ...
    if (action === 'list') {
      ...
      const lines = processes.map((p) => {
        ...
        const runtime =
          p.status === 'running' ? `${Math.round(p.runningMs / 1000)}s` : `${Math.round(p.runningMs / 1000)}s (ended)`;
    ...
    if (action === 'send_input') {
      ...
      return {
        success: sent,
        llmContent: sent ? `Sent input to ${processId}: "${input}"` : `Failed to send input to ${processId}`
      };
    }
