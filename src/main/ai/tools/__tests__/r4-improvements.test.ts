...
describe('R4 Improvements', () => {
  ...
  describe('H-2: search tool', () => {
    ...
    it('搜索文件内容并返回匹配行', async () => {
      ...
      const gen = searchTool.execute(
        { pattern: 'function', searchPath: '.' },
        undefined,
        makeContext(tmpDir)
      )
      const { result } = await consumeGenerator(gen)
  ...
  describe('H-2: glob tool', () => {
    ...
    it('搜索路径穿越被拦截', async () => {
      const gen = globTool.execute(
        { pattern: '*.ts', searchPath: '../../../etc' },
        undefined,
        makeContext(tmpDir)
      )
      const { result } = await consumeGenerator(gen)
  ...
  describe('H-3: exec security fallback', () => {
    ...
    it('安全命令 ls 不被拦截（正常执行）', async () => {
      const gen = execTool.execute(
        { command: 'echo hello-test', timeout: 5000 },
        undefined,
        makeContext(tmpDir)
      )
      const { result } = await consumeGenerator(gen)
