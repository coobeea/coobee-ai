...
function makeContext(workspaceRoot: string, sessionId?: string): ToolExecutionContext {
  return createFallbackToolContext({ workspaceRoot, sessionId });
}
...
describe('Observability & Discovery Tools', () => {
  ...
  describe('session_status', () => {
    ...
    it('无 contexts 目录时返回 0 快照', async () => {
      const gen = sessionStatusTool.execute({}, undefined, makeContext(tmpDir, 'test-session'));
      const { result } = await consumeGenerator(gen);
    ...
    it('正确读取最近一次快照信息', async () => {
      ...
      const gen = sessionStatusTool.execute({}, undefined, makeContext(tmpDir, 'sess-42'));
      const { result } = await consumeGenerator(gen);
      expect(result.success).toBe(true);
    ...
    it('产生 progress yield', async () => {
      const gen = sessionStatusTool.execute({}, undefined, makeContext(tmpDir));
  ...
  describe('session_history', () => {
    ...
    it('无快照时返回空历史', async () => {
      const gen = sessionHistoryTool.execute({}, undefined, makeContext(tmpDir));
      const { result } = await consumeGenerator(gen);
      expect(result.success).toBe(true);
    ...
    it('limit 参数限制条目数', async () => {
      ...
      const gen = sessionHistoryTool.execute({ limit: 2 }, undefined, makeContext(tmpDir));
      const { result } = await consumeGenerator(gen);
      expect(result.success).toBe(true);
  ...
  describe('context_inspect', () => {
    ...
    it('文件不存在时返回错误', async () => {
      ...
      const gen = contextInspectTool.execute({ filename: 'nonexistent.json' }, undefined, makeContext(tmpDir));
      const { result } = await consumeGenerator(gen);
      expect(result.success).toBe(false);
    ...
    it('自动补全 .json 后缀', async () => {
      ...
      const gen = contextInspectTool.execute({ filename: '2026-02-14T10-00-00-000' }, undefined, makeContext(tmpDir));
      const { result } = await consumeGenerator(gen);
      expect(result.success).toBe(true);
  ...
  describe('skill_list', () => {
    ...
    it('正确列出所有 Skill', async () => {
      ...
      const gen = skillListTool.execute({}, undefined, makeContext(tmpDir));
      const { result } = await consumeGenerator(gen);
      expect(result.success).toBe(true);
    ...
    it('包含 filePath 信息（workspace 内显示相对路径）', async () => {
      ...
      const gen = skillListTool.execute({}, undefined, makeContext(skillsDir));
      const { result } = await consumeGenerator(gen);
