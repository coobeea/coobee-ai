...
describe('ProcessRegistry', () => {
  ...
  describe('readOutput()', () => {
    ...
    it('lastN 参数限制行数', async () => {
      const registry = ProcessRegistry.getInstance();
      const { processId } = spawnAndRegister(registry, 'echo a && echo b && echo c && echo d && echo e');
      await wait(80);
  ...
  describe('输出缓冲', () => {
    it('超过 MAX_OUTPUT_LINES 时裁剪', async () => {
      ...
      const { processId } = spawnAndRegister(registry, 'node -e "for(let i=0;i<1100;i++) console.log(\'line\'+i)"');
      await wait(500);
      const output = registry.readOutput(processId);
