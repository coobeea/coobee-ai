...
describe('readTool', () => {
  ...
  describe('正常读取', () => {
    ...
    it('读取空文件', async () => {
      ...
      const { result } = await consumeGenerator(
        readTool.execute({ path: '/tmp/empty.txt' }, undefined, tmpContext)
      )
    ...
    it('读取单行文件', async () => {
      ...
      const { result } = await consumeGenerator(
        readTool.execute({ path: '/tmp/single.txt' }, undefined, tmpContext)
      )
    ...
    it('读取包含中文的文件', async () => {
      ...
      const { result } = await consumeGenerator(
        readTool.execute({ path: '/tmp/chinese.txt' }, undefined, tmpContext)
      )
    ...
    it('读取包含特殊字符的文件', async () => {
      ...
      const { result } = await consumeGenerator(
        readTool.execute({ path: '/tmp/special.txt' }, undefined, tmpContext)
      )
  ...
  describe('错误处理', () => {
    it('文件不存在返回 ENOENT', async () => {
      ...
      const { result } = await consumeGenerator(
        readTool.execute({ path: '/tmp/nope.txt' }, undefined, tmpContext)
      )
    ...
    it('非文件路径返回 NOT_FILE', async () => {
      ...
      const { result } = await consumeGenerator(
        readTool.execute({ path: '/tmp/a_directory' }, undefined, tmpContext)
      )
...
describe('writeTool', () => {
  ...
  describe('正常写入', () => {
    ...
    it('创建嵌套目录', async () => {
      ...
      await consumeGenerator(
        writeTool.execute(
          { path: '/tmp/deep/nested/dir/file.txt', content: 'test' },
          undefined,
          tmpContext
        )
      )
  ...
  describe('参数校验', () => {
    it('content 非字符串返回错误', async () => {
      const { result } = await consumeGenerator(
        writeTool.execute({ path: '/tmp/out.txt', content: 123 })
      )
    ...
    it('content 为 null 返回错误', async () => {
      const { result } = await consumeGenerator(
        writeTool.execute({ path: '/tmp/out.txt', content: null })
      )
    ...
    it('content 为 undefined 返回错误', async () => {
      const { result } = await consumeGenerator(
        writeTool.execute({ path: '/tmp/out.txt', content: undefined })
      )
    ...
    it('content 为对象返回错误', async () => {
      const { result } = await consumeGenerator(
        writeTool.execute({ path: '/tmp/out.txt', content: { key: 'value' } })
      )
...
describe('editTool', () => {
  ...
  describe('正常编辑', () => {
    ...
    it('替换包含中文的文本', async () => {
      ...
      const { result } = await consumeGenerator(
        editTool.execute(
          { path: '/tmp/cn.txt', oldText: '旧的', newText: '新的' },
          undefined,
          tmpContext
        )
      )
  ...
  describe('参数校验', () => {
    it('oldText 未找到返回 NOT_FOUND', async () => {
      ...
      const { result } = await consumeGenerator(
        editTool.execute(
          { path: '/tmp/code.ts', oldText: 'not here', newText: 'replacement' },
          undefined,
          tmpContext
        )
      )
    ...
    it('oldText 未找到但 trim 后能找到给出提示', async () => {
      ...
      const { result } = await consumeGenerator(
        editTool.execute(
          { path: '/tmp/code.ts', oldText: '  hello world  \n', newText: 'new' },
          undefined,
          tmpContext
        )
      )
    ...
    it('多次匹配返回 MULTIPLE_MATCHES', async () => {
      ...
      const { result } = await consumeGenerator(
        editTool.execute(
          { path: '/tmp/code.ts', oldText: 'aaa', newText: 'ccc' },
          undefined,
          tmpContext
        )
      )
    ...
    it('文件不存在返回 ENOENT', async () => {
      ...
      const { result } = await consumeGenerator(
        editTool.execute(
          { path: '/tmp/nope.ts', oldText: 'old', newText: 'new' },
          undefined,
          tmpContext
        )
      )
    ...
    it('权限不足返回 EACCES', async () => {
      ...
      const { result } = await consumeGenerator(
        editTool.execute(
          { path: '/tmp/prot.ts', oldText: 'old', newText: 'new' },
          undefined,
          tmpContext
        )
      )
  ...
  describe('流式输出', () => {
    it('编辑过程有 progress 和 output 更新', async () => {
      ...
      const { updates } = await consumeGenerator(
        editTool.execute(
          { path: '/tmp/s.ts', oldText: 'old text', newText: 'new text' },
          undefined,
          tmpContext
        )
      )
...
describe('文件工具沙箱集成', () => {
  ...
  it('readTool 拒绝越界路径', async () => {
    const { result } = await consumeGenerator(
      readTool.execute({ path: '/etc/passwd' }, undefined, sandboxContext)
    )
    expect(result.success).toBe(false)
    expect(result.error?.code).toBe('SANDBOX_VIOLATION')
  })
  ...
  it('editTool 拒绝越界路径', async () => {
    const { result } = await consumeGenerator(
      editTool.execute(
        { path: '../../../etc/hosts', oldText: 'old', newText: 'new' },
        undefined,
        sandboxContext
      )
    )
  ...
  it('writeTool 拒绝绝对路径越界', async () => {
    const { result } = await consumeGenerator(
      writeTool.execute(
        { path: '/root/.ssh/authorized_keys', content: 'bad' },
        undefined,
        sandboxContext
      )
    )
  ...
  it('sandboxRoot 更严格限制', async () => {
    ...
    const { result } = await consumeGenerator(
      readTool.execute({ path: '../package.json' }, undefined, strictContext)
    )
    expect(result.success).toBe(false)
    expect(result.error?.code).toBe('SANDBOX_VIOLATION')
  })
  ...
  it('允许 sandbox 内的正常操作', async () => {
    ...
    const { result } = await consumeGenerator(
      readTool.execute({ path: 'src/index.ts' }, undefined, sandboxContext)
    )
    expect(result.success).toBe(true)
  })
})
