...
// Mock sandbox
vi.mock('../../sandbox', () => ({
  resolveWorkingDirectory: () => '/mock/workspace'
}));
...
describe('processTool', () => {
  ...
  describe('action=read_output', () => {
    it('正常读取进程输出', async () => {
      ...
      await consumeGenerator(
        execTool.execute({
          command: "node -e \"console.log('line1'); console.log('line2'); setInterval(()=>{}, 9999)\"",
          background: true
        })
      );
    ...
    it('支持 lastN 参数限制返回行数', async () => {
      await consumeGenerator(
        execTool.execute({
          command: 'node -e "for(let i=1;i<=10;i++) console.log(\'line\'+i); setInterval(()=>{}, 9999)"',
          background: true
        })
      );
  ...
  describe('参数校验', () => {
    ...
    it('缺少 processId 时返回错误（send_input）', async () => {
      const { result } = await consumeGenerator(processTool.execute({ action: 'send_input', input: 'test' }));
