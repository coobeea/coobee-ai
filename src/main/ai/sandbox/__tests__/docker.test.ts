 ...
 */
function mockSpawnResult(options: { stdout?: string; stderr?: string; exitCode?: number; error?: Error }): void {
 ...
 */
function mockSpawnSequence(calls: Array<{ stdout?: string; stderr?: string; exitCode?: number; error?: Error }>): void {
...
describe('isDockerAvailable', () => {
  ...
  it('调用 docker info 命令', async () => {
    ...
    expect(mockSpawnFn).toHaveBeenCalledWith('docker', ['info'], expect.objectContaining({ timeout: 5000 }));
  });
});
