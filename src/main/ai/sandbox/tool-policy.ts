 ...
 */
export function isToolAllowed(toolName: string, policy?: SandboxToolPolicy | ResolvedToolPolicy): boolean {
 ...
 * 传入顺序：[最高优先级, ..., 最低优先级]
 */
export function mergeToolPolicies(...policies: (SandboxToolPolicy | undefined)[]): SandboxToolPolicy {
  const deny: string[] = [];
