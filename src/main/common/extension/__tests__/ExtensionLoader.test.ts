...
vi.mock('@main/common/env', () => ({
  Env: {
    ...
    paths: {
      ...
      builtinExtensionsDir: '/mock/ext/builtin',
      userExtensionsDir: '/mock/ext/user'
    },
    getExtensionSearchPaths: vi.fn().mockResolvedValue([])
  }
}));

import { ExtensionRegistry } from '../ExtensionRegistry';
import { ExtensionLoader } from '../ExtensionLoader';
import { log as mockLog } from '@main/common/logger';
...
function createValidExtension(baseDir: string, id: string, opts?: { registerFn?: string }): string {
  ...
  // extension.json
  fs.writeFileSync(path.join(extDir, 'extension.json'), JSON.stringify({ id, name: `Test ${id}`, version: '1.0.0' }));
...
describe('ExtensionLoader', () => {
  ...
  it('load 无 extension.json — 跳过并 warn', async () => {
    ...
    expect(loader.getLoadedIds()).toHaveLength(0);
    expect(mockLog.warn).toHaveBeenCalledWith(expect.stringContaining('no extension.json'));
  });
  ...
  it('load register 抛错 — 该 Extension 跳过，清理已注册内容', async () => {
    ...
    expect(mockLog.error).toHaveBeenCalledWith(expect.stringContaining('register() failed'), expect.any(Error));
  });
  ...
  it('load 无入口文件且无 skills — 只有 extension.json，跳过', async () => {
    ...
    expect(mockLog.warn).toHaveBeenCalledWith(expect.stringContaining('No entry file or skills declaration'));
  });
  ...
  it('skills 目录不存在 — 警告但不阻止加载', async () => {
    ...
    expect(mockLog.warn).toHaveBeenCalledWith(expect.stringContaining('Skill dir declared but not found'));
  });
