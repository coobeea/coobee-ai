...
describe('Gateway 动态方法', () => {
  ...
  it('Gateway 核心方法保护 — chat.* 等不可注册', () => {
    ...
    expect(() => gw.registerMethod('stream.subscribe', handler)).toThrow('namespace "stream" is protected');
    expect(() => gw.registerMethod('worker.start', handler)).toThrow('namespace "worker" is protected');
    expect(() => gw.registerMethod('hitl.decide', handler)).toThrow('namespace "hitl" is protected');
    expect(() => gw.registerMethod('system.methods', handler)).toThrow('namespace "system" is protected');
  });
