    ...
    this.shortcuts.forEach((shortcut) => {
      ...
      if (shortcut.global) {
        ...
        try {
          ...
          log.warn(`[ShortcutManager] 本地快捷键注册失败: ${shortcut.key} -> ${shortcut.shortcut}`, error);
        }
      }
    });

    log.info(`[ShortcutManager] 快捷键注册完成，共注册 ${this.shortcuts.filter((s) => s.registered).length} 个快捷键`);
  }
