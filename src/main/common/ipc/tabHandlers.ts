...
export function registerTabHandlers(): void {
  ...
  ipcMain.handle(
    TabChannels.CREATE,
    async (event, req: CreateTabRequest): Promise<IpcResult<CreateTabResponse>> => {
      ...
      const windowId = req.windowId ?? win.id
  ...
  ipcMain.handle(
    TabChannels.CLOSE,
    async (event, req: CloseTabRequest): Promise<IpcResult<void>> => {
      const win = BrowserWindow.fromWebContents(event.sender)
      if (!win) {
  ...
  ipcMain.handle(
    TabChannels.SWITCH,
    async (event, req: SwitchTabRequest): Promise<IpcResult<void>> => {
      const win = BrowserWindow.fromWebContents(event.sender)
      if (!win) {
  ...
  ipcMain.handle(
    TabChannels.UPDATE,
    async (event, req: UpdateTabRequest): Promise<IpcResult<void>> => {
      const win = BrowserWindow.fromWebContents(event.sender)
      if (!win) {
