...
export class HttpServer {
  ...
  private _registerHttpRoutes(): void {
    ...
    for (const { path: relativePath, module } of discoveredModules) {
      ...
      moduleRegisteredCount += this._registerFunctionRoutes(module as Record<string, unknown>, channelPrefix);
  ...
  private _registerClassRoutes(
    ...
    log.debug(`[HttpServer] Found ${metadata.length} decorated methods in class from ${relativePath}`);
  ...
  private _registerFunctionRoutes(module: Record<string, unknown>, channelPrefix: string): number {
    ...
    for (const exportKey in module) {
      ...
      if (this._createKoaRoute(routePath, exportedValue as (...args: unknown[]) => Promise<unknown>, 'POST')) {
        registeredCount++;
      }
    }
  ...
  private _createKoaRoute(
    ...
    if (this.registeredRoutes.has(routeKey)) {
      log.error(`[HttpServer] Duplicate HTTP route registration detected for "${routeKey}". Registration skipped.`);
      return false;
    }
  ...
  private _createSSERoute(
    ...
    if (this.registeredRoutes.has(routeKey)) {
      log.error(`[HttpServer] Duplicate SSE route registration detected for "${routeKey}". Registration skipped.`);
      return false;
    }
  ...
  private _createStreamRoute(
    ...
    if (this.registeredRoutes.has(routeKey)) {
      log.error(`[HttpServer] Duplicate Stream route registration detected for "${routeKey}". Registration skipped.`);
      return false;
    }
    ...
    try {
      ...
      const routeHandler = async (ctx: any): Promise<void> => {
        ...
        try {
          ...
          const generator = await handlerAdapter.executeStreamFunction(streamFunc, requestData, context);
    ...
    try {
      for await (const streamData of generator) {
        ...
        log.debug(`[HttpServer] Processing chunk ${chunkCount}:`, {
          hasChunk: streamData && typeof streamData === 'object' && 'chunk' in (streamData as object),
          dataType: typeof streamData,
          dataKeys: streamData && typeof streamData === 'object' ? Object.keys(streamData as object) : 'N/A'
        });
        ...
        if (!hasSetHeaders) {
          ...
          if (data.metadata?.cacheControl) {
            ...
            headers['Cache-Control'] =
              disposition === 'inline' ? 'public, max-age=31536000, immutable' : 'no-cache, no-store, must-revalidate';
          }
      ...
      log.info(`[HttpServer] Stream processing completed: ${chunkCount} chunks, ${totalBytes} bytes total`);
    } catch (error: unknown) {
      const err = error as Error & { status?: number };
