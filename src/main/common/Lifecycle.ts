...
import { LifecyclePhase, LifecycleHook, LifecycleContext, LifecycleHookExecutionResult } from './types';
...
export class LifecycleManager {
  ...
  registerHook(hook: LifecycleHook): string {
    ...
    log.debug(`[LifecycleManager] 注册 Hook: ${hook.name} (阶段: ${hook.phase}, 优先级: ${hook.priority})`);
  ...
  async executePhase(phase: LifecyclePhase): Promise<void> {
    ...
    for (const priority of priorityGroups.keys()) {
      ...
      log.debug(`[LifecycleManager] 执行优先级组: ${priority} (${groupHooks.length} 个 Hook)`);
  ...
  private async executeHook(
    ...
    log.debug(`[LifecycleManager] 执行 Hook: ${hook.name} (优先级: ${hook.priority}, 关键: ${hook.critical})`);
    ...
    try {
      const result = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error(`Hook "${hook.name}" 超时（${HOOK_TIMEOUT_MS}ms）`)), HOOK_TIMEOUT_MS)
        )
      ]);

      log.debug(`[LifecycleManager] Hook 完成: ${hook.name}`);
