...
describe('Gateway', () => {
  ...
  describe('handleMessage (RPC)', () => {
    ...
    it('方法不存在返回 METHOD_NOT_FOUND', async () => {
      ...
      await mockCaptured.onMessage!(
        ws,
        JSON.stringify({ type: 'req', id: 'req-2', method: 'nonexist.action' }),
        meta
      )
    ...
    it('handler 抛出 GatewayMethodError 返回结构化错误', async () => {
      ...
      gateway.registerMethods({
        ...
        methods: {
          fail: vi
            .fn()
            .mockRejectedValue(
              new GatewayMethodError(GatewayErrorCode.SESSION_BUSY, 'Session in use')
            )
        }
      })
      ...
      await mockCaptured.onMessage!(
        ws,
        JSON.stringify({ type: 'req', id: 'req-4', method: 'test.fail' }),
        meta
      )
    ...
    it('handler 抛出普通 Error 返回 INTERNAL_ERROR', async () => {
      ...
      await mockCaptured.onMessage!(
        ws,
        JSON.stringify({ type: 'req', id: 'req-5', method: 'test.crash' }),
        meta
      )
  ...
  describe('system.methods', () => {
    it('返回所有已注册方法列表', async () => {
      ...
      await mockCaptured.onMessage!(
        ws,
        JSON.stringify({ type: 'req', id: 'req-sys', method: 'system.methods' }),
        meta
      )
