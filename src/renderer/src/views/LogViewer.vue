...
const categoryOptions: (LogCategory | 'all')[] = ['all', 'event', 'ipc', 'window', 'tab', 'app', 'system', 'user'];
  ...
  <div class="flex h-screen flex-col bg-[#f7f7f8]">
    ...
    <header
      ...
      <div class="flex items-center gap-3 text-[11px]">
        ...
        <span v-if="stats.error > 0" class="font-medium text-red-500"> {{ stats.error }} 错误 </span>
        <span v-if="stats.warn > 0" class="font-medium text-amber-500"> {{ stats.warn }} 警告 </span>
    ...
    <div class="flex shrink-0 items-center gap-3 border-b border-gray-200/80 bg-white px-5 py-2">
      ...
      <div class="relative flex-1">
        <span
          class="i-carbon-search pointer-events-none absolute left-2 top-1/2 inline-block h-3.5 w-3.5 -translate-y-1/2 text-gray-300"></span>
        <input
          v-model="searchText"
          type="text"
          placeholder="搜索日志..."
    ...
    <main ref="logContainer" class="flex-1 overflow-y-auto font-mono text-[12px] leading-5">
      <!-- 空状态 -->
      <div v-if="logStore.filteredLogs.length === 0" class="flex h-full flex-col items-center justify-center">
      ...
      <div class="px-4 py-2">
        <div
          ...
          <!-- 分类 -->
          <span class="mt-px shrink-0 text-[10px] font-medium" :class="categoryStyles[entry.category]">
            [{{ entry.category }}]
          </span>
