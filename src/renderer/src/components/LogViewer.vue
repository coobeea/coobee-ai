...
<template>
  <div class="fixed bottom-4 right-4 z-50">
    ...
    <!-- 日志面板（简化版） -->
    <div v-if="isOpen" class="flex h-[400px] w-[500px] flex-col rounded-lg bg-white shadow-2xl dark:bg-gray-900">
      <!-- 头部 -->
      ...
      <div class="flex-1 overflow-y-auto p-2">
        <div v-if="logStore.logs.length === 0" class="flex h-full items-center justify-center text-sm text-gray-500">
          暂无日志
        </div>
        <div v-else class="space-y-1">
          <div
            ...
            <div class="flex items-start gap-2">
              <span class="font-mono text-[10px] text-gray-500 flex-shrink-0">{{ formatTime(log.timestamp) }}</span>
              <span :class="['font-semibold uppercase text-[10px] flex-shrink-0', levelColors[log.level]]">
                {{ log.level }}
              </span>
              <span class="rounded bg-gray-200 px-1.5 py-0.5 text-[10px] flex-shrink-0 dark:bg-gray-700">
                {{ log.category }}
              </span>
