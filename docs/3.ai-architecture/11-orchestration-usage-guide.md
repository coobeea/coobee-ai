...
// 创建计划版本 1
const version = await planVersionManager.createPlanVersion(plan, PlanVersionReason.INITIAL, '初始任务规划');
...
async function executeTaskWithVerification(task) {
  ...
  // 3. 生成初始计划
  const plan = await planner.plan(task);
  const planVersion = await planVersionManager.createPlanVersion(plan, PlanVersionReason.INITIAL, '初始任务规划');
