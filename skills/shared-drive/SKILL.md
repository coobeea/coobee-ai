...
### 场景 1：完成任务后保存产出（推荐标准流程）

完成重要任务后，**必须**将成果写入共享网盘。系统会自动发送 `shared-drive:entry-created` 事件，任务分发员智能体会自动分析成果并判断是否需要通知其他智能体。

```
1. 执行任务（如市场调研）
2. POST /gateway/shared-drive/entries 创建条目
   - agentId: 使用你自己的 Agent ID
   - topic: 简明描述（如 market-analysis-q1-2026）
   - content: 完整的任务成果内容
   - tags: 便于搜索的标签
   - summary: 一句话概述（其他智能体靠这个判断是否需要查看详情）
3. POST /gateway/shared-drive/entries/{id}/files 上传附件（可选）
4. 系统自动触发后续：
   - 条目创建事件自动广播
   - task-dispatcher 智能体分析成果
   - 相关智能体自动收到跟进任务
```
...
- **写入即通知**：条目创建后系统自动发送事件，任务分发员会自动分析并决定是否需要跟进
