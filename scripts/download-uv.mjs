...
// 支持镜像源（通过环境变量覆盖）
const BASE_URL =
  process.env.UV_MIRROR_URL || `https://github.com/astral-sh/uv/releases/download/${UV_VERSION}`
  ...
  try {
    ...
    if (filename.endsWith('.tar.gz')) {
      ...
      if (existsSync(uvInExtractedDir)) {
        ...
        throw new Error(
          `Could not find ${exeName} executable in the extracted archive for ${platform}-${arch}`
        )
      }
