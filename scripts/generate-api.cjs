...
function parseFileExports(fileContent) {
  ...
  const functionMatches = fileContent.matchAll(/^export\s+(?:(async\s+\*?)\s*)?function\s+(\w+)\s*\(/gm);
  ...
  if (classMatch) {
    ...
    while ((decoratorMatch = decoratorPattern.exec(fileContent)) !== null) {
      ...
      if (methodMatch) {
        ...
        if (methodName !== 'constructor' && !methodName.startsWith('_') && !reservedKeywords.includes(methodName)) {
          if (isGenerator) {
            if (decoratorType === 'Stream') {
...
async function generateApiClient() {
  ...
  for (const filePath of tsFiles) {
    ...
    if (exports.functions.length > 0 || exports.sseFunctions.length > 0 || exports.streamFunctions.length > 0) {
